...
/* ********************************************* */
class LiquidBall extends Particle2D {
    constructor(
        readonly p5: P5,
...
const sketch = (p5: P5) => {
    ...
    p5.setup = () => {
        ...
        for (let i = 0; i < 2; i++) {
            collection.push(
                new LiquidBall(p5, collection, {
                    pos: p5.createVector(innerWidth / 2 + i * 150, 0),
    ...
    p5.draw = () => {
        ...
        p5.fill('#77779d');
        p5.noStroke();
        p5.rect(0, innerHeight / 2, innerWidth, innerHeight / 2);
        collection.forEach((item) => item.update(p5.deltaTime, { drag: true }).draw(p5.deltaTime));
    };
};
